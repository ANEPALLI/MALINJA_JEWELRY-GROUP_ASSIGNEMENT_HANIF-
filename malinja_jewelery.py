...
def open_order_jewelry():
    ...
    def update_item_list():
            mycursor.execute("SELECT item_value, product_value FROM order_jewelry")
            items = mycursor.fetchall()
            item_listbox.delete(0, tk.END)
            for item in items:
                item_listbox.insert(tk.END, f"{item[0]} - RM{item[1]}")
    ...
    def update_order_jewelry():
        ...
        try:
            new_product_value = int(new_product_value)
        except ValueError:
            messagebox.showerror("Error", "Please enter a valid product price (numeric value).")
            return

        # Extract item_value from the selected item
        item_value = selected_item.split(" - ")[0]
    ...
    def calculate():
        ...
        mycursor.execute("SELECT item_value FROM order_jewelry WHERE item_value = %s", (item_value,))
        existing_item = mycursor.fetchone()

        if existing_item:
            # If the item exists, update the quantity and product value
            update_sql = "UPDATE order_jewelry SET product_value = %s, quantity_value = %s WHERE item_value = %s"
            update_val = (product_value, quantity_value, item_value)
            mycursor.execute(update_sql, update_val)
            mydb.commit()

            messagebox.showinfo("Jewelry Info", f"Record for {item_value} updated successfully.")
        else:
...
# in open order jewelry, to update need to inserted the new item, price and quantity first, after that click the chosen item that has been inserted to database in white listbox, and click update.